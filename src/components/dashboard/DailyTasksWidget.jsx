...
function suggestFrequency(plant) {
  ...
  const lightMod = { full_sun: 0.7, partial_sun: 0.85, partial_shade: 1, full_shade: 1.2, artificial: 1.1 }
...
export default function DailyTasksWidget({ plants }) {
  ...
  const { mutate: markWatered, variables: markingVars, isPending } = useMutation({
    mutationFn: async ({ plant, schedule }) => {
    ...
    onMutate: async ({ plant, schedule }) => {
      await queryClient.cancelQueries({ queryKey: ['watering_schedules'] });
      const previous = queryClient.getQueryData(['watering_schedules']);
      const today = format(new Date(), 'yyyy-MM-dd');

      queryClient.setQueryData(['watering_schedules'], (old = []) => {
        if (schedule) {
          return old.map(s => s.id === schedule.id ? { ...s, last_watered_date: today } : s);
        }
        // Temporary optimistic entry
        return [...old, { id: `_temp_${plant.id}`, plant_id: plant.id, frequency_days: suggestFrequency(plant), last_watered_date: today }];
      });
      return { previous };
    },
    onError: (_err, _vars, context) => {
      if (context?.previous) queryClient.setQueryData(['watering_schedules'], context.previous);
    },
    onSettled: () => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
      <div className="flex items-center justify-between px-5 py-3 border-b border-gray-50 select-none">
      ...
      <div className="divide-y divide-gray-50">
        {tasks.map(({ plant, schedule, status, daysOverdue }) => {
          const isMarking = isPending && markingVars?.plant?.id === plant.id;
          return (
            <div key={plant.id} className="flex items-center gap-3 px-5 min-h-[44px] hover:bg-gray-50 transition-colors select-none">
              {plant.image_url ? (
              ...
              <div className="flex-1 min-w-0 py-3">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-xs text-gray-400">Every {schedule?.frequency_days || suggestFrequency(plant)}d</span>
                </div>
              </div>
              ...
              <button
                ...
                disabled={isMarking}
                className="flex items-center gap-1.5 px-3 min-h-[44px] rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-medium transition-colors disabled:opacity-50"
              >
