...
const FunctionDisplay = ({ toolCall }) => {
  ...
  const statusConfig = {
    ...
    completed:   isError ? { icon: AlertCircle,  color: 'text-red-500',   text: 'Failed' }
        ...
        {message.file_urls?.length > 0 && (
          <div className="flex gap-2 mt-1.5 flex-wrap">
            {message.file_urls.map((url, i) => (
              <img key={i} src={url} alt="" className="w-24 h-24 rounded-xl object-cover border border-slate-200" />
            ))}
          </div>
        )}
