      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mt-6">
        ...
        {!user ? (
          <div className="h-10 flex items-center">
            <div className="w-4 h-4 border-2 border-[#52796F]/30 border-t-[#52796F] rounded-full animate-spin" />
          </div>
        ) : ['active', 'trialing'].includes(user.subscription_status) || user.role === 'admin' ? (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 capitalize">{user.role === 'admin' ? 'Admin access' : user.subscription_status === 'trialing' ? 'Free trial active' : 'Active subscription'}</p>
              </div>
            </div>
            {user.role !== 'admin' && (
              <Link to="/Pricing">
